...
module.exports = {
  ...
  send: async (entry) => {
    ...
    if (entry.payload) {
      payload = {
        ...payload,
        data: { notificationId: entry.id.toString(), ...entry.payload },
      };
    }
    ...
    if (entry.targetType === "tokens") {
      ...
      if (tokens.length > 1) {
        res = await admin
          ...
          .sendEachForMulticast(
            { tokens, ...payload, apns: { payload: { aps: options } } },
            false
          );
      } else {
      ...
      if (topics.length > 1) {
        ...
        res = await admin
          .messaging()
          .sendToTopic(entry.target, payload, options);
      }
    }
  ...
  initialize: async (strapi) => {
    ...
    const data = await strapi.db
      .query("plugin::strapi-plugin-fcm.fcm-plugin-configuration")
      .findOne({
