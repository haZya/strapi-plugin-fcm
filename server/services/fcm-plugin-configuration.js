...
module.exports = ({ strapi }) => ({
  ...
  async create(params = {}) {
    ...
    if (count < 1) {
      ...
      return await strapi.entityService.update(uid, data.id, {
        ...params,
        data,
      });
    }
    return {
      error:
        "Only one configuration is allowed, try passing the id to update the existing one.",
    };
  },
  ...
  async update(entityId, params = {}) {
    ...
    if (count < 1) {
      ...
      return await strapi.entityService.update(uid, entityId, {
        ...params,
        data,
      });
    }
